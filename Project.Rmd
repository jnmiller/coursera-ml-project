...
[Citation](http://groupware.les.inf.puc-rio.br/har)
...
a somewhat bizarre and unrealistic formulation of the problem, though it may be 
useful for providing real-time feedback.
...
I discarded these rows prior to training my model:

```{r, cache=TRUE}
library(caret)
setwd("~/coursera/machlearn/project/")
...
#print(paste("PCA at .95 resulted in ", pcaFit$numComp, "components."))
...
for all classes), but the overall accuracy was only `r round( max( bayesPCAFit$results[, "Accuracy"]), 4)`.
...
This was much better, but I wondered if I could increase accuracy even more by discarding PCA and using all the original predictors, without overfitting too much.
